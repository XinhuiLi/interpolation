...
def plot_fnc(fnc, ax, title=None, title_color='k', title_fontsize=44, show_xticks=False, xticks_fontsize=24):
    network_ind = [4.5, 6.5, 15.5, 24.5, 41.5, 48.5]
    ax.imshow(fnc, vmin=-1, vmax=1, cmap="seismic")
    if title:
...
def plot_dwell_time(ax, mean_pt, ste_pt, mean_hc, ste_hc, pval, color_pt, label_pt, title_label, n_state=5, title_fontsize=16, label_fontsize=14, tick_fontsize=12):
    ...
    ax.fill_between(np.arange(1,n_state+1), mean_hc-ste_hc, mean_hc+ste_hc, alpha=0.4, color='cornflowerblue', label='HC')
    ...
    for idx, p in enumerate(pval):
        ...
        asterisk = convert_pvalue_to_asterisks(p)
...
def plot_transition_matrix(ax, transition_matrix, title_label, n_state=5, title_fontsize=16, label_fontsize=14, tick_fontsize=12):
    mask = np.zeros((n_state,n_state))
    np.fill_diagonal(mask,1)
    label = np.arange(1,n_state+1)

    sns.heatmap(transition_matrix,mask=mask,cmap="magma",annot=True,fmt=".2f",vmin=0,vmax=1,xticklabels=label,yticklabels=label,ax=ax, annot_kws={"size": tick_fontsize})
    ax.set_title(title_label, fontsize=title_fontsize)
