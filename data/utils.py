...
def load_sz_score(filename):
    ...
    for i in range(n_sub):
      if score[i,0]==2 and score[i,1]==-9999: #for positive PANSS, if HC has -9999, assign minimal possible score 7
        score[i,1] = 7
      if score[i,0]==2 and score[i,2]==-9999: #for negative PANSS, if HC has -9999, assign minimal possible score 7
        score[i,2] = 7
...
def load_asd_score(filename):
    ...
    """
    data_dict = mat73.loadmat(filename)
    keys = ['DX_GROUP', 'ADOS_TOTAL', 'AGE_AT_SCAN', 'SEX'] # DX: 1 ASD, 2 HC; ADOS: Autism Diagnostic Observation Schedule
    ind = [data_dict['FILE_ID'].index(i) for i in keys]
...
def compute_dwell_state(kmeans_label_2d, sorted_state_ind, n_pt, n_hc, n_state=5):
    ...
    for i in range(n_state):
        ...
        _, dwell_state_pvalue[i] = stats.ttest_ind(a=dwell_state_pt, b=dwell_state_hc, equal_var=True)
...
def compute_transition_matrix(kmeans_label_2d, sorted_state_ind, n_pt, n_state=5, n_window=137):
    ...
    return transition_matrix_pt, transition_matrix_hc
